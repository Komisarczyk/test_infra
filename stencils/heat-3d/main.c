...
extern void heat(
    ...
    int64_t a_offset, int64_t a_sizes0, int64_t a_sizes1, int64_t a_sizes2,
    int64_t a_strides0, int64_t a_strides1, int64_t a_strides2,

    float *b_allocatedptr, float *b_alignedptr,
    int64_t b_offset, int64_t b_sizes0, int64_t b_sizes1, int64_t b_sizes2,
    int64_t b_strides0, int64_t b_strides1, int64_t b_strides2);
  ...
  for (int t = 1; t <= 5; t++)
  {
    for (int i = 1; i < b->sizes[0] - 1; i++)
    {
      for (int j = 1; j < b->sizes[0] - 1; j++)
      {
        ...
        {
          vec_f3d_set(b, i, j, k, (0.125f) * (vec_f3d_get(a, i + 1, j, k) - 2.0f * vec_f3d_get(a, i, j, k) + vec_f3d_get(a, i - 1, j, k)) + (0.125f) * (vec_f3d_get(a, i, j + 1, k) - 2.0f * vec_f3d_get(a, i, j, k) + vec_f3d_get(a, i, j - 1, k)) + (0.125f) * (vec_f3d_get(a, i, j, k + 1) - 2.0f * vec_f3d_get(a, i, j, k) + vec_f3d_get(a, i, j, k - 1)) + vec_f3d_get(a, i, j, k));
        }
      }
    }
    for (int i = 1; i < b->sizes[0] - 1; i++)
    {
      for (int j = 1; j < b->sizes[0] - 1; j++)
      {
        for (int k = 1; k < b->sizes[0] - 1; k++)
        {
          vec_f3d_set(a, i, j, k, (0.125f) * (vec_f3d_get(b, i + 1, j, k) - 2.0f * vec_f3d_get(b, i, j, k) + vec_f3d_get(b, i - 1, j, k)) + (0.125f) * (vec_f3d_get(b, i, j + 1, k) - 2.0f * vec_f3d_get(b, i, j, k) + vec_f3d_get(b, i, j - 1, k)) + (0.125f) * (vec_f3d_get(b, i, j, k + 1) - 2.0f * vec_f3d_get(b, i, j, k) + vec_f3d_get(b, i, j, k - 1)) + vec_f3d_get(b, i, j, k));
        }
      }
    }
  }
...
int main(int argc, char **argv)
{
  ...
  if (vec_f3d_alloc(&a, n, n, n) || vec_f3d_alloc(&a_ref, n, n, n) || vec_f3d_alloc(&b, n, n, n))
  {
  ...
  heat(VEC3D_ARGS(&a), VEC3D_ARGS(&b));
