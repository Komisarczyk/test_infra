#include "../../../polybench-c-4.2.1-beta/stencils/jacobi-2d/jacobi-2d.h"
#include "../../memref.h"
#include <stdio.h>
#include <string.h>
  ...
  for (int t = 0; t < TSTEPS; t++) {
    for (int i = 1; i < b->sizes[0] - 1; i++)
      for (int j = 1; j < b->sizes[1] - 1; j++)
        ...
        vec_f2d_set(b, i, j,
                    0.2f *
                        (vec_f2d_get(a, i, j) + vec_f2d_get(a, i, j - 1) +
                         vec_f2d_get(a, i, j + 1) + vec_f2d_get(a, i + 1, j) +
                         vec_f2d_get(a, i - 1, j)));
    for (int i = 1; i < a->sizes[0] - 1; i++)
      for (int j = 1; j < b->sizes[1] - 1; j++)
        ...
        vec_f2d_set(a, i, j,
                    0.2f *
                        (vec_f2d_get(b, i, j) + vec_f2d_get(b, i, j - 1) +
                         vec_f2d_get(b, i, j + 1) + vec_f2d_get(b, i + 1, j) +
                         vec_f2d_get(b, i - 1, j)));
  }
}
...
int main(int argc, char **argv) {
  ...
  scop_entry(VEC2D_ARGS(&a), VEC2D_ARGS(&b));
