...
void mm_refimpl(struct vec_f1d *a, struct vec_f1d *b)
{
  ...
  for (int t = 0; t < 500; t++)
  {
    for (int i = 1; i < b->sizes[0] - 1; i++)
      ...
      vec_f1d_set(b, i, 0.33333f * (vec_f1d_get(a, i - 1) + vec_f1d_get(a, i) + vec_f1d_get(a, i + 1)));
    for (int i = 1; i < a->sizes[0] - 1; i++)
      vec_f1d_set(a, i, 0.33333f * (vec_f1d_get(b, i - 1) + vec_f1d_get(b, i) + vec_f1d_get(b, i + 1)));
  }
}
...
int main(int argc, char **argv)
{
  ...
  if (vec_f1d_alloc(&a, n) || vec_f1d_alloc(&a_ref, n) || vec_f1d_alloc(&b, n))
  {
  ...
  jacobioned(VEC1D_ARGS(&a), VEC1D_ARGS(&b));
