  ...
  for (int t = 0; t <= 5 - 1; t++)
    for (int i = 1; i <= a->sizes[0] - 2; i++)
      for (int j = 1; j <= a->sizes[1] - 2; j++)
        ...
        vec_f2d_set(a, i, j, (vec_f2d_get(a, i - 1, j - 1) + vec_f2d_get(a, i - 1, j) + vec_f2d_get(a, i - 1, j + 1)
                              //   + A[i][j-1] + A[i][j] + A[i][j+1]
                              + vec_f2d_get(a, i, j - 1) + vec_f2d_get(a, i, j) + vec_f2d_get(a, i, j + 1)
                              //   + A[i+1][j-1] + A[i+1][j] + A[i+1][j+1])/(9.0f);
                              + vec_f2d_get(a, i + 1, j - 1) + vec_f2d_get(a, i + 1, j) + vec_f2d_get(a, i + 1, j + 1)) /
                                 9.0f);
}
