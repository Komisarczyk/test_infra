#include "../../../polybench-c-4.2.1-beta/linear-algebra/blas/symm/symm.h"
#include "../../memref.h"
#include <stdio.h>
#include <string.h>
  ...
  for (int i = 0; i < a->sizes[0]; i++) {
    for (int j = 0; j < c->sizes[1]; j++) {
      ...
      for (int k = 0; k < i; k++) {
        vec_f2d_set(c, k, j,
                    vec_f2d_get(c, k, j) +
                        alpha * vec_f2d_get(b, i, j) * vec_f2d_get(a, i, k));
        temp2 += vec_f2d_get(b, k, j) * vec_f2d_get(a, i, k);
      }
      vec_f2d_set(c, i, j,
                  beta * vec_f2d_get(c, i, j) +
                      alpha * vec_f2d_get(b, i, j) * vec_f2d_get(a, i, i) +
                      alpha * temp2);
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  if (vec_f2d_alloc(&a, n, n) || vec_f2d_alloc(&b, n, m) ||
      vec_f2d_alloc(&c, n, m) || vec_f2d_alloc(&c_ref, n, m)) {
