...
void mm_refimpl(struct vec_f2d *a, struct vec_f2d *b, struct vec_f2d *c)
{
  ...
  for (int i = 0; i < a->sizes[0]; i++)
  {
    ...
    for (int k = 0; k < a->sizes[1]; k++)
      for (int j = 0; j <= i; j++)
      {
        vec_f2d_set(c, i, j, vec_f2d_get(c, i, j) + vec_f2d_get(a, j, k) * alpha * vec_f2d_get(b, i, k) + vec_f2d_get(b, j, k) * alpha * vec_f2d_get(a, i, k));
      }
  }
}
...
int main(int argc, char **argv)
{
  ...
  if (vec_f2d_alloc(&a, n, m) || vec_f2d_alloc(&b, n, m) || vec_f2d_alloc(&c, n, n) || vec_f2d_alloc(&c_ref, n, n))
  {
  ...
  syr2k(VEC2D_ARGS(&a), VEC2D_ARGS(&b), VEC2D_ARGS(&c), 1, 2);
