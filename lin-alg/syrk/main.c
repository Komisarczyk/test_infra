...
void mm_refimpl(struct vec_f2d *a, struct vec_f2d *c)
{
  ...
  for (int i = 0; i < c->sizes[0]; i++)
  {
    ...
    {
      for (int j = 0; j <= i; j++)
        vec_f2d_set(c, i, j, vec_f2d_get(c, i, j) + alpha * vec_f2d_get(a, i, k) * vec_f2d_get(a, j, k));
    }
  }
}
...
int main(int argc, char **argv)
{
  ...
  if (vec_f2d_alloc(&a, n, m) || vec_f2d_alloc(&c, n, n) || vec_f2d_alloc(&c_ref, n, n))
  {
  ...
  syrk(VEC2D_ARGS(&a), VEC2D_ARGS(&c), 1, 2);
