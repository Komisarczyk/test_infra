...
void mm_refimpl(struct vec_f2d *a, struct vec_f2d *b, struct vec_f1d *tmp, struct vec_f1d *x, struct vec_f1d *y_ref)
{
  ...
  for (int i = 0; i < tmp->sizes[0]; i++)
  {
    ...
    for (int j = 0; j < a->sizes[1]; j++)
    {
      ...
      vec_f1d_set(y_ref, i, vec_f2d_get(b, i, j) * vec_f1d_get(x, j) + vec_f1d_get(y_ref, i));
    }
    vec_f1d_set(y_ref, i, alpha * vec_f1d_get(tmp, i) + beta * vec_f1d_get(y_ref, i));
  }
}
...
int main(int argc, char **argv)
{
  ...
  if (vec_f2d_alloc(&a, n, n) || vec_f2d_alloc(&b, n, n) || vec_f1d_alloc(&tmp, n) ||
      vec_f1d_alloc(&x, n) || vec_f1d_alloc(&y, n) || vec_f1d_alloc(&y_ref, n))
  {
  ...
  gesummv(VEC2D_ARGS(&a), VEC2D_ARGS(&b), 1.2f, 1.3f, VEC1D_ARGS(&tmp), VEC1D_ARGS(&x), VEC1D_ARGS(&y));
