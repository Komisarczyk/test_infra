...
extern void gemver(
    ...
    float *u1_allocatedptr, float *u1_alignedptr,
    int64_t u1_offset, int64_t u1_sizes0, int64_t u1_strides0,

    float *v1_allocatedptr, float *v1_alignedptr,
    int64_t v1_offset, int64_t v1_sizes0, int64_t v1_strides0,

    float *u2_allocatedptr, float *u2_alignedptr,
    int64_t u2_offset, int64_t u2_sizes0, int64_t u2_strides0,

    float *v2_allocatedptr, float *v2_alignedptr,
    int64_t v2_offset, int64_t v2_sizes0, int64_t v2_strides0,

    float *w_allocatedptr, float *w_alignedptr,
    int64_t w_offset, int64_t w_sizes0, int64_t w_strides0,
    ...
    float *z_allocatedptr, const float *z_alignedptr,
    int64_t z_offset, int64_t z_sizes0, int64_t z_strides0);

/* Reference implementation of a matrix multiplication */
void mm_refimpl(struct vec_f2d *a, struct vec_f1d *u1, struct vec_f1d *v1, struct vec_f1d *u2, struct vec_f1d *v2,
                struct vec_f1d *x, struct vec_f1d *y, struct vec_f1d *z, struct vec_f1d *w_ref)
{
  ...
  for (int i = 0; i < a->sizes[0]; i++)
    for (int j = 0; j < a->sizes[1]; j++)
      vec_f2d_set(a, i, j, vec_f2d_get(a, i, j) + vec_f1d_get(u1, i) * vec_f1d_get(v1, j) + vec_f1d_get(u2, i) * vec_f1d_get(v2, j));

  for (int i = 0; i < x->sizes[0]; i++)
    for (int j = 0; j < a->sizes[0]; j++)
      vec_f1d_set(x, i, vec_f1d_get(x, i) + beta * vec_f2d_get(a, j, i) * vec_f1d_get(y, j));
  ...
  for (int i = 0; i < w_ref->sizes[0]; i++)
    for (int j = 0; j < a->sizes[1]; j++)
    {
      vec_f1d_set(w_ref, i, vec_f1d_get(w_ref, i) + alpha * vec_f2d_get(a, i, j) * vec_f1d_get(x, j));
    }
}
...
int main(int argc, char **argv)
{
  ...
  if (vec_f2d_alloc(&a, n, n) || vec_f1d_alloc(&u1, n) || vec_f1d_alloc(&v1, n) ||
      vec_f1d_alloc(&u2, n) || vec_f1d_alloc(&v2, n) || vec_f1d_alloc(&x, n) || vec_f1d_alloc(&y, n) || vec_f1d_alloc(&z, n) || vec_f1d_alloc(&w, n) ||
      vec_f1d_alloc(&w_ref, n))
  {
  ...
  gemver(VEC2D_ARGS(&a), 1.2f, 1.3f, VEC1D_ARGS(&u1), VEC1D_ARGS(&v1), VEC1D_ARGS(&u2), VEC1D_ARGS(&v2), VEC1D_ARGS(&w), VEC1D_ARGS(&x),
