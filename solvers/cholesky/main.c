...
void mm_refimpl(struct vec_f2d *a)
{
  ...
  for (int i = 0; i < a->sizes[0]; i++)
  {
    ...
    for (int j = 0; j < i; j++)
    {
      for (int k = 0; k < j; k++)
      {
        vec_f2d_set(a, i, j, vec_f2d_get(a, i, j) - vec_f2d_get(a, i, k) * vec_f2d_get(a, j, k));
      }
    ...
    for (int k = 0; k < i; k++)
    {
      vec_f2d_set(a, i, i, vec_f2d_get(a, i, i) - vec_f2d_get(a, i, k) * vec_f2d_get(a, i, k));
    }
