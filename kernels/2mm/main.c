...
extern void twomm(
    ...
    int64_t tmp_offset, int64_t tmp_sizes0, int64_t tmp_sizes1,
    int64_t tmp_strides0, int64_t tmp_strides1

);

/* Reference implementation of a matrix multiplication */
void mm_refimpl(struct vec_f2d *a, struct vec_f2d *b, struct vec_f2d *c, struct vec_f2d *d, struct vec_f2d *tmp)
{
  ...
  for (int i = 0; i < tmp->sizes[0]; i++)
    for (int j = 0; j < tmp->sizes[1]; j++)
    {
      ...
      for (int k = 0; k < a->sizes[1]; ++k)
        vec_f2d_set(tmp, i, j, vec_f2d_get(tmp, i, j) + alpha * vec_f2d_get(a, i, k) * vec_f2d_get(b, k, j));
    }
  for (int i = 0; i < d->sizes[0]; i++)
    for (int j = 0; j < d->sizes[1]; j++)
    {
      ...
      for (int k = 0; k < tmp->sizes[1]; ++k)
        vec_f2d_set(d, i, j, vec_f2d_get(d, i, j) + vec_f2d_get(tmp, i, k) * vec_f2d_get(c, k, j));
    }
}
...
int main(int argc, char **argv)
{
  ...
  if (vec_f2d_alloc(&a, n, p) || vec_f2d_alloc(&b, p, m) || vec_f2d_alloc(&c, m, r) || vec_f2d_alloc(&d, n, r) || vec_f2d_alloc(&d_ref, n, r) || vec_f2d_alloc(&tmp, n, m))
  {
  ...
  twomm(VEC2D_ARGS(&a), VEC2D_ARGS(&b), VEC2D_ARGS(&c), VEC2D_ARGS(&d), 2, 3, VEC2D_ARGS(&tmp));
