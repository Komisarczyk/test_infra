#include "../../../polybench-c-4.2.1-beta/linear-algebra/kernels/atax/atax.h"
#include "../../memref.h"
#include <stdio.h>
#include <string.h>
...
void mm_refimpl(struct vec_f2d *a, struct vec_f1d *tmp, struct vec_f1d *x,
                struct vec_f1d *y_ref) {
  ...
  for (int i = 0; i < tmp->sizes[0]; i++) {
    ...
    for (int j = 0; j < y_ref->sizes[0]; j++)
      vec_f1d_set(y_ref, j, vec_f1d_get(tmp, i) * vec_f2d_get(a, i, j) +
                                vec_f1d_get(y_ref, j));
  }
}
...
int main(int argc, char **argv) {
  ...
  if (vec_f2d_alloc(&a, n, m) || vec_f1d_alloc(&tmp, n) ||
      vec_f1d_alloc(&x, m) || vec_f1d_alloc(&y, m) ||
      vec_f1d_alloc(&y_ref, m)) {
  ...
  scop_entry(VEC2D_ARGS(&a), VEC1D_ARGS(&tmp), VEC1D_ARGS(&x), VEC1D_ARGS(&y));
