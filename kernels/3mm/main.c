#include "../../../polybench-c-4.2.1-beta/linear-algebra/kernels/3mm/3mm.h"
#include "../../memref.h"
#include <stdio.h>
#include <string.h>
...
extern void scop_entry(
    ...
    float *e_allocatedptr, float *e_alignedptr, int64_t e_offset,
    int64_t e_sizes0, int64_t e_sizes1, int64_t e_strides0, int64_t e_strides1,

    float *f_allocatedptr, float *f_alignedptr, int64_t f_offset,
    int64_t f_sizes0, int64_t f_sizes1, int64_t f_strides0, int64_t f_strides1,

    float *g_allocatedptr, float *g_alignedptr, int64_t g_offset,
    int64_t g_sizes0, int64_t g_sizes1, int64_t g_strides0, int64_t g_strides1

    );

/* Reference implementation of a matrix multiplication */
void mm_refimpl(struct vec_f2d *a, struct vec_f2d *b, struct vec_f2d *c,
                struct vec_f2d *d, struct vec_f2d *e, struct vec_f2d *f,
                struct vec_f2d *g) {
  ...
  for (int i = 0; i < e->sizes[0]; i++)
    for (int j = 0; j < e->sizes[1]; j++) {
      ...
      for (int k = 0; k < a->sizes[1]; ++k)
        vec_f2d_set(e, i, j, vec_f2d_get(e, i, j) +
                                 vec_f2d_get(a, i, k) * vec_f2d_get(b, k, j));
    }
  /* F := C*D */
  for (int i = 0; i < f->sizes[0]; i++)
    for (int j = 0; j < f->sizes[1]; j++) {
      ...
      for (int k = 0; k < c->sizes[1]; ++k)
        vec_f2d_set(f, i, j, vec_f2d_get(f, i, j) +
                                 vec_f2d_get(c, i, k) * vec_f2d_get(d, k, j));
    }
  /* G := E*F */
  for (int i = 0; i < g->sizes[0]; i++)
    for (int j = 0; j < g->sizes[1]; j++) {
      ...
      for (int k = 0; k < e->sizes[1]; ++k)
        vec_f2d_set(g, i, j, vec_f2d_get(g, i, j) +
                                 vec_f2d_get(e, i, k) * vec_f2d_get(f, k, j));
    }
}
...
int main(int argc, char **argv) {
  ...
  if (vec_f2d_alloc(&a, n, p) || vec_f2d_alloc(&b, p, m) ||
      vec_f2d_alloc(&c, m, q) || vec_f2d_alloc(&d, q, r) ||
      vec_f2d_alloc(&e, n, m) || vec_f2d_alloc(&f, m, r) ||
      vec_f2d_alloc(&g, n, r) || vec_f2d_alloc(&g_ref, n, r)) {
  ...
  scop_entry(VEC2D_ARGS(&a), VEC2D_ARGS(&b), VEC2D_ARGS(&c), VEC2D_ARGS(&d),
             VEC2D_ARGS(&e), VEC2D_ARGS(&f), VEC2D_ARGS(&g));
